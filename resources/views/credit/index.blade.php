...
<div class="card-header text-center">
  <h4 class="text-center" style="background: #1D62F0; color: #fff; padding: 10px;">CREDITS</h4>
</div>
<br>
<div class="row">
    <div class="col-md-4 col-sm-3">
      <a href="{{ route('credit.reglement')}}"
          class=" btn btn-sm btn-primary" style="margin-left: 5%; box-shadow: 0px 0px 15px #95A5A6; background: #1D62F0; color: #fff;"><i class="fa fa-plus"></i>En régle</a>
    </div>
</div>
<br>
<div class="card-body">
    ...
    <tbody id="tbody">
      @if($credits->count() > 0)
        @foreach($credits as $c)
          @if($c->montant != $c->versement)
            <tr>
              <td>{{ Carbon\Carbon::parse($c->date)->day. '-'. Carbon\Carbon::parse($c->date)->month. '-'. Carbon\Carbon::parse($c->date)->year. ' à ' . Carbon\Carbon::parse($c->date)->hour.':'. Carbon\Carbon::parse($c->date)->minute. ':'. Carbon\Carbon::parse($c->date)->second }}</td>
              <td> {{ $c->client }}</td>
              <td> {{ $c->telephone }}</td>
              <td> {{ $c->montant }}</td>
              <td> {{ $c->versement }}</td>
              <td>
                <form class="" action="{{ route('crediter.versement') }}" method="post">
                  {{ csrf_field() }}
                  <input type="hidden" name="client" value="{{ $c->client }}">
                  <input type="hidden" name="telephone" value="{{ $c->telephone }}">
                  <input type="hidden" name="date" value="{{ $c->date }}">
                  <input type="text" class="form-control" name="versement" value="">
                  <input type="submit" class="btn btn-success" name="" value="valider">
                </form>
                <form class="" action="{{ route('crediter.details') }}" method="post">
                  {{ csrf_field() }}
                  <input type="hidden" name="client" value="{{ $c->client }}">
                  <input type="hidden" name="telephone" value="{{ $c->telephone }}">
                  <input type="hidden" name="date" value="{{ $c->date }}">
                  <input type="submit" class="btn btn-primary" name="" value="Détails">
                </form>
              </td>
            </tr>
          @endif
