...
<div class="card-header text-center">
  <h4 class="text-center" style="background: #1D62F0; color: #fff; padding: 10px;">REGLEMENTS</h4>
</div>
<br>
<div class="card-body">
      ...
      @if($reglements->count() > 0)
        @foreach($reglements as $r)
          <tr>
            ...
            <td> {{ $r->total }}</td>
            <td>
              <form action="{{ route('reglement.confirmer')}}" method="post">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="place" value="{{$r->place}}">
                <input type="hidden" name="serveuse" value="{{$r->serveuse}}">
                <input type="hidden" name="m_apayer" value="{{ $r->total }}">
                <input class="form-control" type="text" name="m_recu" value="" placeholder="Saisir le montant donné par le client !">
                <input class="btn btn-success" type="submit" name="" value="Confirmer">
              </form>
            </td>
          </tr>
          @endforeach
        @else
          <tr>
            <td colspan="4" class="text-center"> Aucune addition à régler !</td>
          </tr>
        @endif
    </tbody>
